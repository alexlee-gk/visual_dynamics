...
class TheanoServoingPolicy(ServoingPolicy):
    ...
    def _get_A_b_c_split_vars(self):
        ...
        vars_ = L.get_output([self.predictor.pred_layers[name] for name in utils.flatten_tree(names)], deterministic=True)
        feature_vars, jac_vars, next_feature_vars = utils.unflatten_tree(names, vars_)
        ...
        feature_shapes = L.get_output_shape([self.predictor.pred_layers[name] for name in utils.flatten_tree(self.predictor.feature_name)])
