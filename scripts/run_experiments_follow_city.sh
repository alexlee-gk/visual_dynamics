...
if [ "$#" -eq 0 ]; then
    ...
    echo "Usage: ./run_experiments_follow_city.sh [FACTOR]"
    exit
fi
echo "FACTOR=${FACTOR}"
...
N=20
T=20
DATA=data/city32_train_data_0-4.h5
VAL_DATA=data/city32_val_data.h5
MAX_ITER=10000
ARGS="${DATA} ${VAL_DATA} --share ${SHARE} --x1_c_dim ${X1_C_DIM} --lr 0.001 -p fcn_action_cond_encoder_net -t $T -n $N -v0 --max_iter ${MAX_ITER} --concat 1 --pf auto --no_train --experiment 1 --dof_limit_factor ${FACTOR} --output_results_dir results"
SIM_ARGS="city"
