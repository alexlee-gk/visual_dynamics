...
import utils
from algorithms import Algorithm, ServoingOptimizationAlgorithm
...
class CrossEntropyMethodAlgorithm(Algorithm):
    ...
    """
    def __init__(self, f, th_mean, batch_size, sampling_iters, elite_frac=0.2, initial_std=1.0, th_low=None, th_high=None):
        """
...
class ServoingCrossEntropyMethodAlgorithm(ServoingOptimizationAlgorithm, CrossEntropyMethodAlgorithm):
    def __init__(self, env, servoing_pol, sampling_iters,
                 batch_size=None, elite_frac=0.2, initial_std=None,
        ...
        CrossEntropyMethodAlgorithm.__init__(self,
                                             ...
                                             batch_size=batch_size or min(3 * len(self.servoing_pol.theta), 100),
                                             sampling_iters=sampling_iters,
                                             elite_frac=elite_frac,
                                             initial_std=initial_std or self.servoing_pol.theta,
                                             th_low=0.0)
    ...
    def _noisy_evaluation(self, theta):
        ...
        _, _, _, rewards = utils.do_rollouts(self.env, self.noisy_pol, self.num_trajs, self.num_steps,
                                             seeds=np.arange(self.num_trajs))
        # restore servoing parameters
        self.servoing_pol.w, self.servoing_pol.lambda_ = w, lambda_
        return np.mean(utils.discount_returns(rewards, self.gamma))
