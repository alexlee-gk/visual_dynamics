...
# Conv2DLayer = lambda incoming, num_filters, name=None: LT.GroupConv2DLayer(incoming, num_filters, groups=num_filters, name=name, **conv2d_kwargs)
Conv2DLayer = lambda incoming, num_filters, name=None: CC.Conv2DCCLayer(incoming, 512, groups=32, name=name, **conv2d_kwargs)

# img_channels = 8192
# numGroups = 16
# filter_channels = 8192 // numGroups
# numGroups * filter_channels != img_channels

# if ((num_filters %% (numGroups * 16)) != 0)
# "Each group must have a multiple of 16 channels, but num_filters %%%%
# (numGroups * 16) = %%d %%%% ( %%d * 16) = %%d.",
# num_filters, numGroups, num_filters % % (numGroups * 16));
# ValueError: Each group must have a multiple of 16 channels, but num_filters %
# (numGroups * 16) = 512 % ( 512 * 16) = 512.
# num_filters = 512
# numGroups = 512
#
...
l_xu = LT.OuterProductLayer([l_x, l_u], name='xu')
l_y = Conv2DLayer(l_xu, x_shape[0], name='y')
...
Y_grad_var = theano.grad((Y_var ** 2).mean(axis=0).sum(), L.get_all_params(l_y))
